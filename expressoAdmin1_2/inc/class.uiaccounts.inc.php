...
include_once(PHPGW_API_INC.'/class.aclmanagers.inc.php');
	...
	class uiaccounts
	{
		...
		function add_users()
		{
			...
			// Pega combo das organiza��es e seleciona, caso seja um post, o setor que o usuario selecionou.
		...
		function edit_user()
		{
			...
			// SOMENTE ALTERA��O DOS ATRIBUTOS SAMBA
			...
			if (count($user_info['groups_info']) > 0)
			{
				...
				{
					// O memberUid do usu�rio est� somente no Banco, ent�o adicionamos o memberUid no Ldap.
				...
				// O memberUid do usu�rio est� somente no Ldap.
				...
				foreach ($user_info['groups_ldap'] as $gidnumber=>$cn)
				{
					if (is_null($groups_db[$gidnumber]))
					{
						...
						if ($alert_warning == '')
							$alert_warning = "O expressoAdmin corrigiu as seguintes inconsist�ncias:\\n";
						$alert_warning .= "Removido atributo memberUid do usu�rio do grupo $cn.\\n";
						*/
