...
final class Process
{
    private $oProc;
    private $tables;
    private $updateincluded = array();
    private $translation;
    
    public function setup_process()
    {
    ...
    public function init_process()
    {
        ...
        $phpgw_domain = GlobalService::get('phpgw_domain');
        
        GlobalService::get('phpgw_setup')->oProc = CreateObject('phpgwapi.schema_proc',$phpgw_domain[$ConfigDomain]['db_type']);
        GlobalService::get('phpgw_setup')->oProc->m_odb           = GlobalService::get('phpgw_setup')->db;
        GlobalService::get('phpgw_setup')->oProc->m_odb->Host     = $phpgw_domain[$ConfigDomain]['db_host'];
        GlobalService::get('phpgw_setup')->oProc->m_odb->Database = $phpgw_domain[$ConfigDomain]['db_name'];
        GlobalService::get('phpgw_setup')->oProc->m_odb->User     = $phpgw_domain[$ConfigDomain]['db_user'];
        GlobalService::get('phpgw_setup')->oProc->m_odb->Password = $phpgw_domain[$ConfigDomain]['db_pass'];
        GlobalService::get('phpgw_setup')->oProc->m_odb->Halt_On_Error = 'report';
        GlobalService::get('phpgw_setup')->oProc->m_odb->connect();
    }
     ...
     */
    public function pass($setup_info,$method='new',$DEBUG=False,$force_en=False)
    {
        if(!$method)
        {
            return False;
        ...
        while($pass_string != $passing_string)
        {
            ...
            $setup_info = GlobalService::get('phpgw_setup')->detection->compare_versions($setup_info);
            //_debug_array($setup_info);exit;
            $setup_info = GlobalService::get('phpgw_setup')->detection->check_depends($setup_info);
    ...
    /*!
     @public function save_minimal_config
     @abstract saves a minimal default config, so you get a running install without entering and saveing Step #2 config
     */
    public function save_minimal_config()
    {
        GlobalService::get('current_config')['site_title'] = 'eGroupWare';
        GlobalService::get('current_config')['hostname']  = $_SERVER['HTTP_HOST'];
        // files-dir is not longer allowed in document root, for security reasons !!!
        GlobalService::get('current_config')['files_dir'] = '/outside/webserver/docroot';
        
        if(@is_dir('/tmp'))
        {
            GlobalService::get('current_config')['temp_dir'] = '/tmp';
        }
        elseif(@is_dir('c:\\temp'))
        {
            GlobalService::get('current_config')['temp_dir'] = 'c:\\temp';
        }
        else
        {
            GlobalService::get('current_config')['temp_dir'] = '/path/to/temp/dir';
        }
        // guessing the phpGW url
        $parts = explode('/',$_SERVER['PHP_SELF']);
        array_pop($parts);	// remove config.php
        array_pop($parts);	// remove setup
        GlobalService::get('current_config')['webserver_url'] = implode('/',$parts);
        
        $datetime = CreateObject('phpgwapi.date_time');
        GlobalService::get('current_config')['tz_offset'] = $datetime->getbestguess();
        unset($datetime);
        
        // RalfBecker: php.net recommend this for security reasons, it should be our default too
        GlobalService::get('current_config')['usecookies'] = 'True';
        
        foreach(GlobalService::get('current_config') as $setting => $value)
        {
            $setting = GlobalService::get('phpgw_setup')->db->db_addslashes($setting);
            $value   = GlobalService::get('phpgw_setup')->db->db_addslashes($value);
            @GlobalService::get('phpgw_setup')->db->query("DELETE FROM phpgw_config WHERE config_app='phpgwapi' AND config_name='$setting'",__LINE__,__FILE__);
            GlobalService::get('phpgw_setup')->db->query("INSERT INTO phpgw_config (config_app,config_name, config_value) VALUES ('phpgwapi','$setting','$value')");
        }
    }
    ...
    {
        if(!@GlobalService::get('phpgw_setup')->oProc)
        {
            $this->init_process();
        }
        GlobalService::get('phpgw_setup')->oProc->m_bDeltaOnly = False;
        
        /* The following is built so below we won't try to drop a table that isn't there. */
        $tablenames = GlobalService::get('phpgw_setup')->db->table_names();
            ...
            if(is_array($data['tables']))
            {
                foreach($data['tables'] as $table)
                {
                    ...
                    if(in_array($table,$tables))
                    {
                        ...
                        GlobalService::get('phpgw_setup')->oProc->DropTable($table);
    ...
    public function current($setup_info,$DEBUG=False)
    {
        ...
        while(list($key,$null) = @each($setup_info))
        {
            ...
            if($setup_info[$key]['tables'] && file_exists($appdir.'tables_current.inc.php'))
            {
                ...
                if($ret)
                {
                    if(GlobalService::get('phpgw_setup')->app_registered($appname))
                    {
                        GlobalService::get('phpgw_setup')->update_app($appname);
                        GlobalService::get('phpgw_setup')->update_hooks($appname);
                    }
                    else
                    {
                        GlobalService::get('phpgw_setup')->register_app($appname);
                        GlobalService::get('phpgw_setup')->register_hooks($appname);
                    }
                    // Update the array values for return below
            ...
            else
            {
                ...
                {
                    GlobalService::get('phpgw_setup')->register_app($appname,$enabled);
                    GlobalService::get('phpgw_setup')->register_hooks($appname);
                }
                $setup_info[$key]['status'] = 'C';
            }
    ...
    /*!
     @public function default_records
    ...
    public function default_records($setup_info,$DEBUG=False)
    {
        ...
        while(list($key,$null) = @each($setup_info))
        {
            ...
            if($setup_info[$key]['tables'] && file_exists($appdir.'default_records.inc.php'))
            {
                ...
                GlobalService::get('phpgw_setup')->oProc->m_odb->transaction_begin();
                $oProc = &GlobalService::get('phpgw_setup')->oProc;	// to be compatible with old apps
                include ($appdir.'default_records.inc.php');
                GlobalService::get('phpgw_setup')->oProc->m_odb->transaction_commit();
            }
            /* $setup_info[$key]['status'] = 'C'; */
        }
    ...
    /*!
     ...
     @param $setup_info	array of application info from setup.inc.php files, etc.
     */
    public function add_langs($setup_info,$DEBUG=False,$force_langs=False)
    {
        @reset($setup_info);
        while(list($key,$null) = @each($setup_info))
        {
    ...
    public function baseline($setup_info,$DEBUG=False)
    {
        ...
        while(list($key,$null) = @each($setup_info))
        {
            ...
            if(file_exists($appdir.'tables_baseline.inc.php'))
            {
                ...
                GlobalService::get('phpgw_setup')->oProc->GenerateScripts($phpgw_baseline, $DEBUG);
    ...
    public function upgrade($setup_info,$DEBUG=False)
    {
        ...
        GlobalService::get('phpgw_setup')->oProc->m_odb->HaltOnError = 'no';
        GlobalService::get('phpgw_setup')->oProc->m_bDeltaOnly = True;
        
        @reset($setup_info);
        while(list($key,$null) = @each($setup_info))
        {
            /* Don't try to upgrade an app that is not installed */
            if(!GlobalService::get('phpgw_setup')->app_registered($setup_info[$key]['name']))
            {
                if($DEBUG)
                {
                ...
                $setup_info[$key]['status'] == '') // TODO this is not getting set for api upgrade, sometimes ???
            {
                ...
                $test   = array();
                GlobalService::get('phpgw_setup')->oProc->m_aTables = $phpgw_baseline = array();
                /*
                 ...
                 GlobalService::get('phpgw_setup')->oProc->m_aTables = $phpgw_baseline;
                ...
                if(file_exists($appdir . 'tables_update.inc.php') && !@$this->updateincluded[$appname])
                {
                    ...
                    while(list($x,$value) = @each($test))
                    {
                        ...
                        /* build upgrade public function name */
                        ...
                        elseif (GlobalService::get('phpgw_setup')->alessthanb($value,$currentver))
                        {
    ...
    public function post_process($tables,$DEBUG=False)
    {
        ...
        $ret = GlobalService::get('phpgw_setup')->oProc->GenerateScripts($tables,$DEBUG);
        if($ret)
        {
            $oret = GlobalService::get('phpgw_setup')->oProc->ExecuteScripts($tables,$DEBUG);
            if($oret)
            {
                return True;
            }
    ...
    public function sql_to_array($tablename='')
    {
        ...
        GlobalService::get('phpgw_setup')->oProc->m_oTranslator->_GetColumns(GlobalService::get('phpgw_setup')->oProc, $tablename, $sColumns, $sColumnName);
        
        while(list($key,$tbldata) = each(GlobalService::get('phpgw_setup')->oProc->m_oTranslator->sCol))
        {
            $arr .= $tbldata;
        }
        $pk = GlobalService::get('phpgw_setup')->oProc->m_oTranslator->pk;
        $fk = GlobalService::get('phpgw_setup')->oProc->m_oTranslator->fk;
        $ix = GlobalService::get('phpgw_setup')->oProc->m_oTranslator->ix;
        $uc = GlobalService::get('phpgw_setup')->oProc->m_oTranslator->uc;
