...
use Expresso\Setup\Process;
use Expresso\Setup\Detection;
...
class Setup
{
    ...
    {
        $this->setup($html, $translation);
    }
    
    /**
     * @todo check which variable is directly read
     * @param string $name
     * @return mixed
     */
    ...
    public function setup($html = false, $translation = false)
    {
        $this->detection = new Detection();
        $this->process   = new Process();
        //$this->appreg    = CreateObject('phpgwapi.app_registry'); TODO remove this line 
        
        /* The setup application needs these */
        $this->html = ($html ? new SetupHtml() : '');
     ...
     */
    public function set_cookiedomain()
    {
     ...
     */
    public function set_cookie($cookiename,$cookievalue='',$cookietime=0)
    {
    ...
    /*!
     ...
     @param	$auth_type	???
     */
    public function auth($auth_type='Config')
    {
        #phpinfo();
    ...
    public function check_auth($user,$pw,$conf_user,$conf_pw)
    {
        if ($user != $conf_user)
        {
    ...
    public function checkip($remoteip='')
    {
    ...
    /*!
     ...
     @param	$
     */
    public function get_major($versionstring)
    {
        if(!$versionstring)
        {
            return False;
     ...
     */
    public function register_app($appname,$enable=99)
    {
     ...
     */
    public function app_registered($appname)
    {
        $setup_info = GlobalService::get('setup_info');
    ...
    /*!
     ...
     @param	$enabled	optional, set to False to not enable this app
     */
    public function update_app($appname)
    {
        $setup_info = GlobalService::get('setup_info');
    ...
    /*!
     ...
     @param	$tableschanged	???
     */
    public function update_app_version($setup_info, $appname, $tableschanged = True)
    {
        if(!$appname)
        {
            return False;
    ...
    /*!
     ...
     @param	$appname	Application 'name' with a matching $setup_info[$appname] array slice
     */
    public function deregister_app($appname)
    {
        if(!$appname)
        {
            return False;
    ...
    /*!
     ...
     @param	$appname	Application 'name' with a matching $setup_info[$appname] array slice
     */
    public function register_hooks($appname)
    {
        $setup_info = GlobalService::get('setup_info');
    ...
    /*!
     ...
     @param	$appname	Application 'name' with a matching $setup_info[$appname] array slice
     */
    public function update_hooks($appname)
    {
    ...
    /*!
     ...
     @param	$appname	Application 'name' with a matching $setup_info[$appname] array slice
     */
    public function deregister_hooks($appname)
    {
     ...
     */
    public function hook($location, $appname='')
    {
        if (!is_object($this->hooks))
        {
            $this->hooks = CreateObject('phpgwapi.hooks',$this->db);
    ...
    /*
     ...
     #return	True if $a < $b
     */
    public function alessthanb($a,$b,$DEBUG=False)
    {
    ...
    /*!
     ...
     #return	True if $a < $b
     */
    public function amorethanb($a,$b,$DEBUG=False)
    {
        $num = array('1st','2nd','3rd','4th');
    ...
    public function get_hooks_table_name()
    {
    ...
    public function setup_account_object()
    {
     ...
     */
    public function add_account($username,$first,$last,$passwd,$group='default',$changepw=False)
    {
     ...
     */
    public function add_acl($apps,$location,$account,$rights=1)
    {
        if (!is_int($account))
        {
            $this->setup_account_object();
