...
class SetupHtml
{
    ...
    function generate_header()
    {
        ...
        GlobalService::get('header_template')->set_file(array(
            'header' => 'header.inc.php.template'
        ));
    ...
    function show_header($title = '', $nologoutbutton = False, $logoutfrom = 'config', $configdomain = '')
    {
        ...
        header('Content-type: text/html; charset=' . Functions::lang('charset'));

        if (!GlobalService::isset('setup_tpl')){
            GlobalService::set('setup_tpl', SetupTplFactory::getInstance());
        }
        GlobalService::get('setup_tpl')->set_var('charset', Functions::lang('charset'));
        $style = array(
            'th_bg' => '#486591',
            'th_text' => '#FFFFFF',
            'row_on' => '#DDDDDD',
        ...
        if ($nologoutbutton) {
            ...
            $btn_logout = '<a href="index.php?FormLogout=' . $logoutfrom . '" class="link">' . Functions::lang('Logout') . '</a>';
            $check_install = '<a class="textsidebox" href="check_install.php">' . Functions::lang('Check installation') . '</a>';
        }

        GlobalService::get('setup_tpl')->set_var('lang_setup', Functions::lang('setup'));
        GlobalService::get('setup_tpl')->set_var('page_title', $title);
        if ($configdomain == '') {
            GlobalService::get('setup_tpl')->set_var('configdomain', '');
        } else {
            GlobalService::get('setup_tpl')->set_var('configdomain', ' - ' . Functions::lang('Domain') . ': ' . $configdomain);
        }

        if (basename($_SERVER['SCRIPT_FILENAME']) != 'index.php') {
            $index_btn = '<a href="index.php" class="link">' . Functions::lang('Setup Main Menu') . '</a>';
            $index_img = '<img src="templates/default/images/orange-ball.png" alt="ball" />';
        }

        GlobalService::get('setup_tpl')->set_var('lang_version', Functions::lang('version'));
        ...
        GlobalService::get('setup_tpl')->set_var(array(
            ...
            'main_menu' => Functions::lang('Setup Main Menu'),
            'user_login' => Functions::lang('Back to user login')
        ));
    ...
    function make_href_link_simple($pre_link_blurb = '', $href_link = '', $href_text = 'default text', $post_link_blurb = '')
    {
        ...
        $simple_link = $pre_link_blurb . '<a href="' . $href_link . '">' . $href_text . '</a> ' . $post_link_blurb . "\n";
        return $simple_link;
    }
