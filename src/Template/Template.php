...
class Template
{
    ...
    /**
     *
     * @private $rb_debug mixed False for no debug, string or array of strings with:
     *	- function-name like set_private to get eg. all assignments or
     *	- handle- / variable-names - if you are only interested in some variables ;-)
     */
    private $debug = False;

    // array('cat_list','cat_list_t');
    ...
    public function set_var($varname, $value = '')
    {
        ...
        foreach ($varname as $k => $v) {
            if (! empty($k)) {
                ...
                $this->varvals[$k] = $this->egroupware_hack ? str_replace(array(
                    'phpGroupWare',
    ...
    public function check_debug($str)
    {
        if (! $this->debug)
            return False;

        foreach (func_get_args() as $arg) {
            if (! is_array($this->debug) && $this->debug === $arg || (is_array($this->debug) && (@$this->debug[$arg] || in_array($arg, $this->debug, True)))) {
                return True;
            }
        }
        return False;
