<?php
	...
	*  option) any later version.                                              *
	...
	if(isset($passwd_type) || $_POST['submitit_x'] || $_POST['submitit_y'] || $submit)
	{
	    ...
	    // Primeiro testa o captcha....se houver......
            if(GlobalService::get('phpgw_info')['server']['captcha']==1)
              {
                if($_SESSION['contador'] > GlobalService::get('phpgw_info')['server']['num_badlogin'])
                    {
		...
		{
			If(!$_GET['cd']) $_GET['cd'] = GlobalService::get('phpgw')->session->cd_reason;
		}
		else
		{
	...
	* And if the register link must be placed                *
	...
	$template = GlobalService::get('phpgw_info')['login_template_set'];
	GlobalService::get('phpgw_info')['server']['template_set'] = $template;
	...
	$template_dir = GlobalService::get('phpgw_info')['server']['webserver_url'] . '/phpgwapi/templates/' . $template;
	$login_dir = $template_dir . '/login.css';
	$login_css = "<link href='" . $login_dir . "' rel='stylesheet' type='text/css' />";
	$login_dir = $template_dir . '/themes/' . GlobalService::get('phpgw_info')['user']['preferences']['common']['theme'] . '/login.css';
	...
	if (!@GlobalService::get('phpgw_info')['server']['login_virtual_keyboard']) 
	...
	if(GlobalService::get('phpgw_info')['server']['captcha'] && $_GET['cd']!='300' )
	{
	...
	if ($_GET['cd']=='300' && GlobalService::get('phpgw_info')['server']['certificado']==1)
	{
...
*/
	...
	{
		if(GlobalService::get('phpgw_info')['server']['certificado']==1)
		{
