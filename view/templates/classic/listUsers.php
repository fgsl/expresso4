<?php			
	...
	{
		$user_context = GlobalService::get('phpgw_info')['server']['ldap_context'];
		system('echo "CONTEXT: '.$user_context.'" >/tmp/controle');
		$organizations_info = $obj_org_sector->organization_search(GlobalService::get('phpgw_info')['server']['ldap_context']);
		@asort($organizations_info);
		@reset($organizations_info);
		$sectors_info = $obj_org_sector->sector_search('ou='.$post_select_organization.','.GlobalService::get('phpgw_info')['server']['ldap_context']);
		@reset($sector_info);
		$cn='*'.$search_users.'*';
	}
	...
	$ldap_host =GlobalService::get('phpgw_info')['server']['ldap_host'];
