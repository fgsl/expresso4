<?php
	...
	*  option) any later version.                                              *
	...
	function parse_navbar($force = False)
	{
		...
		GlobalService::get('celepar_tpl')->set_file(
			array(
				'navbar' => 'navbar.tpl'
			)
		);
		...
		$var['img_root'] = GlobalService::get('phpgw_info')['server']['webserver_url'] . '/phpgwapi/templates/celepar/images';
		...
		foreach(GlobalService::get('phpgw_info')['navbar'] as $app => $app_data)
		{
			...
			if(GlobalService::get('phpgw_info')['user']['preferences']['common']['start_and_logout_icons']!='no')	
			{
				...
				$app_icons .= '<td width="74px" height="40" valign="center" align="center" onclick="location.href=\''.GlobalService::get('phpgw_info')['navbar']['logout']['url'].'\'" style="cursor:pointer;cursor:hand;background-repeat:no-repeat;padding-bottom:20px;background-image:url('.$var['img_root'].'/back_app.png)">'.$icon.'</td>';
			}			//			window.location.href=\''.GlobalService::get('phpgw_info')['navbar']['logout']['url'].'\'
			
			$var['app_icons'] = $app_icons;
			
			$hint = GlobalService::get('phpgw_info')['navbar']['logout']['title'];
			$icon = '<img width="16" src="'.GlobalService::get('phpgw_info')['navbar']['logout']['icon'].'"  alt="'.$hint.'" title="'.$hint.'">';
			...
			if((GlobalService::get('phpgw_info')['user']['agree_terms'] != 1) && (GlobalService::get('phpgw_info')['server']['use_agree_term']=='True')) //Ele dever� confirmar o termo de aceite.
			{
			...
			if((GlobalService::get('phpgw_info')['user']['lastpasswd_change'] == '0') && ((GlobalService::get('phpgw_info')['user']['agree_terms'] == 1) || (GlobalService::get('phpgw_info')['server']['use_agree_term']!='True')))
			{
				$changepasswd_url = $_SERVER['HTTP_HOST'] . GlobalService::get('phpgw_info')['server']['webserver_url'] . '/preferences/changepassword.php?cd=1';
		...
		function sidebox_menu_item($item_link='',$item_text='')
		{
			...
			{
				$var['icon_or_star']='<img src="'.GlobalService::get('phpgw_info')['server']['webserver_url'] . '/phpgwapi/templates/celepar/images'.'/orange-ball.png" width="9" height="9" alt="ball"/>';
				$var['target'] = '';
		...
		function parse_navbar_end()
		{
			...
			$var = Array(
				'img_root'       => GlobalService::get('phpgw_info')['server']['webserver_url'] . '/phpgwapi/templates/celepar/images',
