  ...
  \**************************************************************************/
	...
	class uiholiday
	{
		...
		function uiholiday()
		{
			...
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['calendar']['title'].' - '.lang('Holiday Management');
		}
		...
		function admin()
		{
			...
			$var = Array(
				...
				'left_next_matchs'	=> GlobalService::get('phpgw')->nextmatchs->left('/index.php?menuaction=calendar.uiholiday.admin',$this->bo->start,$this->bo->total),
				'right_next_matchs'	=> GlobalService::get('phpgw')->nextmatchs->right('/index.php?menuaction=calendar.uiholiday.admin',$this->bo->start,$this->bo->total),
				'center'			=> '<td align="center">'.lang('Countries').'</td>',
				'sort_name'		=> GlobalService::get('phpgw')->nextmatchs->show_sort_order($this->bo->sort,'locale',$this->bo->order,'/calendar/'.basename($SCRIPT_FILENAME),lang('Country')),
			...
			else
			{
				...
				while (list(,$value) = each($locales))
				{
					...
					$var = Array(
						'tr_color'		=> $tr_color,
						'group_name'	=> $value,
						'edit_link'		=> '<a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.edit_locale','locale'=>$value)) . '"> '.lang('Edit').' </a>',
						'delete_link'	=> '<a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.delete_locale','locale'=>$value)).'"> '.lang('Delete').' </a>',
						'extra_link'	=> '<a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.submit','locale'=>$value)).'"> '.lang('Submit').' </a>'.
							' &nbsp; &nbsp; <a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.submit','locale'=>$value,'download'=>1)).'"> '.lang('Download').' </a>'
					);
					$p->set_var($var);
			...
			$var = Array(
				'new_action'		=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.edit_holiday','id'=>0)),
				'lang_add'			=> lang('add'),
				'search_action'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.admin')),
		...
		function edit_locale($locale='')
		{
			...
			if(!$this->bo->total && !isset($this->bo->query))
			{
				...
				GlobalService::get('phpgw')->redirect_link($this->base_url,$link_params);
			}
			unset(GlobalService::get('phpgw_info')['flags']['noheader']);
			unset(GlobalService::get('phpgw_info')['flags']['nonavbar']);
			GlobalService::get('phpgw_info')['flags']['noappfooter'] = True;
			...
			$html = &GlobalService::get('phpgw')->html;
			...
			$var = Array(
				'th_bg'				=> GlobalService::get('phpgw_info')['theme']['th_bg'],
				'left_next_matchs'	=> GlobalService::get('phpgw')->nextmatchs->left('/index.php',$this->bo->start,$total,'&menuaction=calendar.uiholiday.edit_locale&locale='.$this->bo->locales[0].'&year='.$this->bo->year),
				'right_next_matchs'	=> GlobalService::get('phpgw')->nextmatchs->right('/index.php',$this->bo->start,$total,'&menuaction=calendar.uiholiday.edit_locale&locale='.$this->bo->locales[0].'&year='.$this->bo->year),
				'center'			=> '<td align="right">'.lang('Holidays').' ('.$this->bo->locales[0].')</td><td align="left">'.$year_form.'</td>',
				'sort_name'			=> GlobalService::get('phpgw')->nextmatchs->show_sort_order($this->bo->sort,'name',$this->bo->order,'/index.php',lang('Holiday'),'&menuaction=calendar.uiholiday.edit_locale&locale='.$this->bo->locales[0].'&year='.$this->bo->year),
				'header_edit'		=> lang('Edit'),
				'header_delete'		=> lang('Delete'),
				'header_rule'		=> '<td>'.GlobalService::get('phpgw')->nextmatchs->show_sort_order($this->bo->sort,'month_num,mday',$this->bo->order,'/index.php',lang('Rule'),'&menuaction=calendar.uiholiday.edit_locale&locale='.$this->bo->locales[0].'&year='.$this->bo->year).'</td>',
			...
			else
			{
				...
				for($i=$this->bo->start; $i < $end; $i++)
				{
					...
					$var = Array(
						...
						'edit_link'		=> '<a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.edit_holiday','locale'=>$this->bo->locales[0],'id'=>$holidays[$i]['index'],'year'=>$this->bo->year)).'"> '.lang('Edit').' </a>',
						'extra_link'	=> '<a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.copy_holiday','locale'=>$this->bo->locales[0],'id'=>$holidays[$i]['index'],'year'=>$this->bo->year)).'"> '.lang('Copy').' </a>',
						'delete_link'	=> '<a href="'.GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.delete_holiday','locale'=>$this->bo->locales[0],'id'=>$holidays[$i]['index'],'year'=>$this->bo->year)).'"> '.lang('Delete').' </a>'
					);
			...
			$var = Array(
				'new_action'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.edit_holiday','locale'=>$this->bo->locales[0],'id'=>0,'year'=>$this->bo->year)),
				'lang_add'		=> lang('add'),
				'back_action'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.admin')),
				'lang_back'		=> lang('Back'),
				'search_action'=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.edit_locale','locale'=>$this->bo->locales[0],'year'=>$this->bo->year)),
				'lang_search'	=> lang('search')
			);
			$p->set_var($var);
		...
		function edit_holiday($error='',$holiday='')
		{
			...
			unset(GlobalService::get('phpgw_info')['flags']['nonavbar']);
			GlobalService::get('phpgw_info')['flags']['noappfooter'] = True;
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['calendar']['title'].' - '.($this->bo->id ? lang('Edit') : lang('Add')).' '.lang('Holiday');
			GlobalService::get('phpgw')->common->phpgw_header();
			...
			if (@count($error))
			{
				$message = GlobalService::get('phpgw')->common->error_list($error);
			}
			else
			{
				$message = '';
			...
			$var = Array(
				...
				'message'       => $message,
				'actionurl'     => GlobalService::get('phpgw')->link($this->base_url,'menuaction=calendar.boholiday.add&year='.$this->bo->year),
...
// Date
			$this->display_item($t,lang('Date'),GlobalService::get('phpgw')->common->dateformatorder($this->sb->getYears('holiday[year]',$holiday['occurence']>1900?$holiday['occurence']:0),$this->sb->getMonthText('holiday[month_num]',$holiday['month']),$this->sb->getDays('holiday[mday]',$holiday['day'])).
...
// Occurence
			...
			$t->set_var(Array(
				'action_url_button'	=> GlobalService::get('phpgw')->link($this->base_url,$link_params),
		...
		function delete_locale()
		{
			...
			$var = Array(
				'action_url_button'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.admin')),
			...
			$var = Array(
				'action_url_button'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.boholiday.delete_locale','locale'=>$this->bo->locales[0])),
		...
		function delete_holiday()
		{
			...
			$var = Array(
				'action_url_button'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.uiholiday.edit_locale','locale'=>$this->bo->locales[0],'year'=>$this->bo->year)),
				'action_text_button'	=> lang('No'),
				'action_confirm_button'	=> '',
				'action_extra_field'	=> ''
			);
			...
			$var = Array(
				'action_url_button'	=> GlobalService::get('phpgw')->link($this->base_url,Array('menuaction'=>'calendar.boholiday.delete_holiday','locale'=>$this->bo->locales[0],'id'=>$this->bo->id,'year'=>$this->bo->year)),
				'action_text_button'	=> lang('Yes'),
				'action_confirm_button'	=> '',
				'action_extra_field'	=> ''
			);
		...
		function submit()
		{
			...
			elseif($this->debug)
			{
				$action = GlobalService::get('phpgw')->link('/calendar/phpgroupware.org/accept_holiday.php');
			}
			else
			{
