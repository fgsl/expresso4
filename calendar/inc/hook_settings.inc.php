...
*/
	...
	for ($i=0; $i < 24; ++$i)
	{
		$options[$i] = GlobalService::get('phpgw')->common->formattime($i,'00');
	}
	...
	$groups = GlobalService::get('phpgw')->accounts->membership(GlobalService::get('phpgw_info')['user']['account_id']);
	$options = array(-1 => lang('none'));
	if (is_array($groups))
	{
		foreach($groups as $group)
		{
			$options[$group['account_id']] = GlobalService::get('phpgw')->common->grab_owner_name($group['account_id']);
		}
	}
...
//		'public+group'   => lang('global public and group public')
	...
	$freebusy_url = GlobalService::get('phpgw_info')['server']['webserver_url'].'/calendar/freebusy.php?user='.GlobalService::get('phpgw_info')['user']['account_lid'].'&password='.GlobalService::get('phpgw_info')['user']['preferences']['calendar']['freebusy_pw'];
	if ($freebusy_url[0] == '/')
	{
