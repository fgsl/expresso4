		...
		*/
		function find($what, $rules=false, $other=false)
		{
			...
			foreach($relacionados as $uid_relacionado => $tipo_relacionamento) {
				...
				$aclTemp->read();
				$perms_relacao = $aclTemp->get_specific_rights(GlobalService::get('phpgw_info')['user']['account_id'],'contactcenter'); //Preciso verificar as permiss�es que o contato relacionado deu para o atual
				
				if(!($perms_relacao&1)) //Se n�o tiver permiss�o de leitura, nem se preocupe em list�-los
					continue;
