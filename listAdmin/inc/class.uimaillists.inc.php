	...
	*  option) any later version.														 *
	...
	class uimaillists
	{
		...
		function uimaillists()
		{
			...
			GlobalService::get('phpgw')->js->validate_file('jscode','connector','listAdmin');#diretorio, arquivo.js, aplicacao
			GlobalService::get('phpgw')->js->validate_file('jscode','expressoadmin','listAdmin');
			GlobalService::get('phpgw')->js->validate_file('jscode','maillists','listAdmin');
		}
		...
		function list_maillists()
		{
			...
			$context = GlobalService::get('phpgw_info')['server']['ldap_context']; //$acl[0]['context'];
			$context_display = ldap_dn2ufn(GlobalService::get('phpgw_info')['server']['ldap_context']); //$acl[0]['context_display'];
			...
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['listAdmin']['title'].' - '.lang('Email Lists');
			...
			$var = Array(
				...
				'add_action'				=> GlobalService::get('phpgw')->link('/index.php','menuaction=listAdmin.uimaillists.add_maillists'),
			...
			if( ($auto_list == "true") && (GlobalService::get('query') == '') )
			{
			...
			//Admin make a search
				if( (GlobalService::get('query') != '') && (strlen(GlobalService::get('query')) < 4) ) {
					$p->set_var('message',lang('Search argument too short'));
				}else if (GlobalService::get('query') != ''){
					//Retorna as listas que o usuario conectado administra, de acordo com o argumento de usca
					$maillists_info = $this->functions->get_list('maillists', GlobalService::get('query'), $contextoListas, $admlista);
				}
			}
		...
		function add_maillists()
		{
			...
			unset(GlobalService::get('phpgw_info')['flags']['noheader']);
			unset(GlobalService::get('phpgw_info')['flags']['nonavbar']);
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['listAdmin']['title'].' - '.lang('Create Email List');
			GlobalService::get('phpgw')->common->phpgw_header();
...
//				'defaultMemberModeration_checked'		=> 'CHECKED',	
				...
				'back_url'					=> GlobalService::get('phpgw')->link('/index.php','menuaction=listAdmin.uimaillists.list_maillists'),
				'combo_org'					=> $org,
		...
		function edit_maillists()
		{
			...
			$manager_context = GlobalService::get('phpgw_info')['server']['ldap_context']; //$acl[0]['context'];
			//$context_display = ldap_dn2ufn(GlobalService::get('phpgw_info')['server']['ldap_context']); //$acl[0]['context_display'];
			...
			unset(GlobalService::get('phpgw_info')['flags']['noheader']);
			unset(GlobalService::get('phpgw_info')['flags']['nonavbar']);
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['listAdmin']['title'].' - '.lang('Edit Email Lists');
			GlobalService::get('phpgw')->common->phpgw_header();
			...
			// Pega combo das organiza��es e seleciona a org da lista.
			...
			if (count($maillist_info['members_info']) > 0)
			{
				...
				if ($unknow != '')
				{
					//$opt_tmp_unknow = '<option  value="-1" disabled>--------------------&nbsp;&nbsp;&nbsp;&nbsp;E-mails n�o encontrados&nbsp;&nbsp;&nbsp;&nbsp;------------------ </option>'."\n";
				...
				$opt_tmp_users  = '<option  value="-1" disabled>-----------------------------&nbsp;&nbsp;&nbsp;&nbsp;Usu�rios&nbsp;&nbsp;&nbsp;&nbsp;---------------------------- </option>'."\n";
...
//			GlobalService::get('phpgw')->js->set_onload('get_available_users_only(document.forms[0].org_context.value, document.forms[0].ea_check_allUsers.checked);');
			...
			unset(GlobalService::get('phpgw_info')['flags']['noheader']);
			unset(GlobalService::get('phpgw_info')['flags']['nonavbar']);
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['listAdmin']['title'].' - '.lang('Admin Lists');
			GlobalService::get('phpgw')->common->phpgw_header();
...
//						$users .= "<option value=" . $uidnumber . ">" . $cn .  " [" . $array_users_uid[$uidnumber] . "]</option>";
			...
			$var = Array(
				...
				'type'						=> 'adm_maillist',
				'ldap_context'					=> GlobalService::get('phpgw_info')['server']['ldap_context'],
				'dn_listas'					=> GlobalService::get('phpgw_info')['server']['dn_listas'],
				'lang_back'					=> lang('Back'),
				'lang_save'					=> lang('save'),
				'lang_org'					=> lang('Organizations'),
				'lang_maillist_uid'				=> lang('Maillist login'),
		...
		function scl_maillists()
		{
			...
			unset(GlobalService::get('phpgw_info')['flags']['noheader']);
			unset(GlobalService::get('phpgw_info')['flags']['nonavbar']);
			GlobalService::get('phpgw_info')['flags']['app_header'] = GlobalService::get('phpgw_info')['apps']['listAdmin']['title'].' - '.lang('Edit Sending Control List');
			GlobalService::get('phpgw')->common->phpgw_header();
