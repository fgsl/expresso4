...
class ScriptS {
    ...
    var $SieveS;   // Var para cria��o do objeto;
    ...
    var $size;  // Tamanho;
    var $so;  // Verifica se a regra foi criada por outro tipo servi�o de filtros;
    var $continuebit;
    var $sizebit;
    var $allofbit;
    var $keepbit;
    ...
    function ScriptS() {
        ...
        //$this->scriptfile = GlobalService::get('HTTP_SESSION_VARS']['phpgw_info']['expressomail']['user']['account_lid'];
    ...
    function init_a() {
        ...
        if (!$this->reply) {
            ...
            if (!$aux) {
                // Caso de erro, grava dentro da vari�vel errstr;
        ...
        if ($this->errstr) {
            ...
            if ($_SESSION['phpgw_info']['server']['expressomail']['expressoMail_enable_log_messages'] == "True")
                error_log(date("D M j G:i:s T Y")
                        ...
                        . " "
                        . $this->SieveS->errstr . "\n", 3, $error_log_file);
            return "Problemas na cria��o do arquivo!\n" . $this->teste;
        }
    }
    ...
    function write_rule() {
        ...
        foreach ($this->newrules as $tmp)
        {
            ...
            if ($rule['status'] == 'ENABLED') 
            {
                ...
                // Condi��es para montagem das regras;
                $anyall = "anyof";
                if ($rule['allof'])
                    $anyall = "allof";
                if ($rule['regexp']) {
        ...
        // L� as regras fora do escrit�rio;
        if ($this->newout != "") {
            $newscriptfoot .= preg_replace("/[\\n\\r]/", " ", $this->newout) . "\n";
        }
...
// Fim da Fun��o
    ...
    function readScript($scriptName) {
        ...
        if (!$scriptName) {
            $this->errstr = "N�o foi poss�vel conectar com o Servidor";
            return "false 2";
        }

        // Recebe o conte�do do array;
        $lines = array();
        $lines = preg_split("/\n/", $scriptName);
        ...
        // Verifica a composi��o do script;
