...
class FsUtils {
	...
	function readFolder ($path)	{
		...
		if (! is_dir($absolute) ) {			
			$this->_appendError("O caminho $absolute n�o � um diret�rio.");
			return false; 
		}
		
		if (! $dir = @opendir($absolute) ) {			
			$this->_appendError("O diret�rio $absolute n�o pode ser lido.");
			return false; 					
		} else {			
			while ( false !== ( $fileName = readdir($dir) ) ) {				
				...
				if ( is_file($file) ) { /* ignores subdirs */										
					if (! is_readable($file) ) {
						$this->_appendError("O arquivo " . $path . $file . " n�o pode ser lido.");
					} else {
	...
	function getConfig ($configFile) {		
		...
		if (! file_exists($absolute) ) {			
			return "O arquivo $configFile n�o existe.";
		} elseif (! is_file($absolute) ) {
			return $configFile . " n�o � um arquivo comum.";
		} elseif (! is_readable($absolute) ) {
			return "O arquivo $configFile n�o pode ser lido.";
		}
