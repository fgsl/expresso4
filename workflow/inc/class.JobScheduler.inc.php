...
/**
 * Classe que verifica se um Job deve ser executado neste momento. E, em caso positivo, dispara sua execu��o
...
class JobScheduler
{
	...
	/**
	 * @var object $db Objeto de conex�o com o Banco de Dados
	 * @access private
	 */
	private $db;
	...
	/**
	 * Tempo m�ximo de execu��o de um Job (em microsegundos)
	...
	/**
	 * Verifica os jobs que devem ser executados e solicita sua execu��o
	 * @return void
	 * @access public
	 */
	public function run()
	{
		...
		for ($i = 0; $i < $numerOfJobs; ++$i)
		{
			...
			{
				$runningJobs[$i]->kill();
				$this->jobManager->writeLog($jobs[$i]['job_id'], $this->currentDate, 'O Job foi abortado por ultrapassar o limite do tempo de execu��o (atualmente em: ' . (JobScheduler::MAXIMUM_EXECUTION_TIME / 1000000) . 's)', JobManager::STATUS_FAIL);
			}
		}
	}

	/**
	 * Dispara a execu��o de um Job
	 * @param array $job Um registro da tabela de jobs
	 * @param bool $testMode Indica se o Job ser� executado em modo de teste (true) ou n�o (false)
	 * @return object Um objeto da classe Thread que gerencia a execu��o do Job
	 * @access public
	 */
...
/* se este arquivo � executado a partir da linha de comando, executa os
 * jobs que est�o programados para execu��o no momento da chamada */
