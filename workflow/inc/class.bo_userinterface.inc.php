...
{
	/**
	 * @var object Acesso � camada Model
	 * @access public
	 */
	var $so;

	/**
	 * @var array Informa��es sobre a organiza��o do usu�rio
	 * @access private
	 */
	...
	/**
	 * Retorna os processos do usu�rio
	 * @access public
	...
	/**
	 * Fornece os dados para a contru��o da interface de Tarefas Pendentes
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return array Contendo os dados para constru��o da interface ou uma mensagem de erro
	 * @access public
	 */
		...
		foreach ($result['data'] as $row)
		{
			...
			if (($row['wf_status'] == 'active') || ($row['wf_status'] == 'exception'))
			{
				...
				$actionsArray[] = ($row['wf_status'] == 'active') ?
					array('name' => 'exception', 'value' => $availableActions['exception'], 'text' => 'Colocar em Exce��o') :
					array('name' => 'resume', 'value' => $availableActions['resume'], 'text' => 'Retirar de Exce��o');
	...
	/**
	 * Fornece os dados para a contru��o da interface de Tarefas Pendentes (quando os dados est�o agrupados)
	 * @return array Contendo os dados para constru��o da interface ou uma mensagem de erro
	 * @access public
	 */
	...
	/**
	 * Envia uma inst�ncia para pr�xima atividade
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return mixed Array contendo uma mensagem de erro ou um booleano (true) informando que a a��o foi feita com sucesso
	 * @access public
	 */
	function inboxActionSend($params)
	{
		...
		if (!$GLOBALS['ajax']->gui->gui_send_instance($activityID, $instanceID))
			$result = array('error' => $GLOBALS['ajax']->gui->get_error(false) . "<br />Voc� n�o est� autorizado a enviar esta inst�ncia.");
	...
	/**
	 * Libera uma inst�ncia (atribui a inst�ncia para *)
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return mixed Array contendo uma mensagem de erro ou um booleano (true) informando que a a��o foi feita com sucesso
	 * @access public
	 */
	function inboxActionRelease($params)
	{
		...
		if (!$GLOBALS['ajax']->gui->gui_release_instance($activityID, $instanceID))
			$result = array('error' => $GLOBALS['ajax']->gui->get_error(false) . "<br />Voc� n�o est� autorizado a liberar esta inst�ncia.");
	...
	/**
	 * Captura uma inst�ncia (atribui a inst�ncia para o usu�rio atual)
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return mixed Array contendo uma mensagem de erro ou um booleano (true) informando que a a��o foi feita com sucesso
	 * @access public
	 */
	function inboxActionGrab($params)
	{
		...
		if (!$GLOBALS['ajax']->gui->gui_grab_instance($activityID, $instanceID))
			$result = array('error' => $GLOBALS['ajax']->gui->get_error(false) . "<br />Voc� n�o tem permiss�o para capturar esta inst�ncia.");
	...
	/**
	 * Transforma a inst�ncia em exce��o
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return mixed Array contendo uma mensagem de erro ou um booleano (true) informando que a a��o foi feita com sucesso
	 * @access public
	 */
	function inboxActionException($params)
	{
		...
		if (!$GLOBALS['ajax']->gui->gui_exception_instance($activityID, $instanceID))
			$result = array('error' => $GLOBALS['ajax']->gui->get_error(false) . "<br />Voc� n�o tem permiss�o para transformar esta inst�ncia em exce��o.");
	...
	/**
	 * Retira uma inst�ncia em exce��o
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return mixed Array contendo uma mensagem de erro ou um booleano (true) informando que a a��o foi feita com sucesso
	 * @access public
	 */
	function inboxActionResume($params)
	{
		...
		if (!$GLOBALS['ajax']->gui->gui_resume_instance($activityID, $instanceID))
			$result = array('error' => $GLOBALS['ajax']->gui->get_error(false) . "<br />Voc� n�o tem permiss�o para retirar de exce��o esta inst�ncia.");
	...
	function inboxActionAbort($params)
	{
		...
		if (!$GLOBALS['ajax']->gui->gui_abort_instance($activityID, $instanceID))
			$result = array('error' => $GLOBALS['ajax']->gui->get_error(false) . "<br />Voc� n�o tem permiss�o para abortar esta inst�ncia.");
	...
	/**
	 * Visualiza dados de uma inst�ncia
	 * @param $params Par�metros advindos da chamada Ajax
	 * @return mixed Array contendo uma mensagem de erro ou um booleano (true) informando que a a��o foi feita com sucesso
	 * @access public
	 */
	...
	/**
	 * Verifica se o usu�rio tem acesso ao Organograma (e se o mesmo est� ativo)
	 * @return mixed true em caso de sucesso ou uma array contendo mensagens sobre o problema (n�o cadastrado ou organograma n�o ativo)
	 * @access private
	 */
	private function checkOrgchartAccess()
	{
		...
		if ($this->organizationInfo === false)
			return array('warning' => 'Voc� n�o est� cadastrado em nenhuma organiza��o');

		if ($this->organizationInfo['ativa'] == 'N')
			return array('warning' => 'Organograma indispon�vel');
	...
	function getCategoryEmployees($params)
	{
		...
		if ($employees === false)
			return array('error' => 'Categoria n�o encontrada.');
	...
	function searchEmployee($params)
	{
		if (!preg_match('/^([[:alnum:] -]+)$/', $params['searchTerm']))
			return array('error' => 'Par�metro de busca inv�lido');
		...
		if (!is_array($result['employees']) and
			...
			return array('error' => 'O sistema de busca n�o pode ser utilizado para sua organiza��o');
	 ...
	 * @param array $params Uma array contendo o ID do funcion�rio cujas informa��es ser�o extra�das.
	 ...
	 * @param array $params Uma array contendo o ID da �rea cujas informa��es ser�o extra�das.
	...
	/**
	 * Retorna a lista de telefones �teis da organiza��o
	...
	/**
	 * Retorna a lista as �reas com substitui��o de chefia
	 * @return array Lista das �reas com substitui��o de chefia
	 * @access public
	 */
	...
	function getAlphabeticalEmployees( )
	{
		...
		if ( $employees === false )
			return array( 'error' => 'Localidade n�o encontrada.' );
