...
if ($acao == "excluir") {
    ...
    pg_query($conn,$sql);
    $msg = "Coluna Exclu�da com sucesso!";
}
...
if ($form->isSubmit("atualizar_coluna",true)) {
    ...
    try {
     ...
     ";
           $res = pg_query($conn,$sql);
           if (!$res) { throw new \Exception("Atualizando informa��es da coluna."); }
...
if(isset($lstoid{0})){
    ...
    $form->adicionarCampo("lslchtml","lslchtml","Html da Listagem:","Html da Listagem",$lslchtml,true,"","","campo que ser� exibido entre chaves. Ex: {nomedocampo}");
    ...
    $form->adicionarCampo("lslcordem","lslcordem","Ordem:","Ordem da Coluna",$lslcordem,true,"10","","Ordem que a coluna ir� aparecer.");
    ...
    $form->adicionarCampo("lslclink_condicao","lslclink_condicao","Link Condi��o:","Link Condi��o",$lslclink_condicao,false,"","","Condi��o para exibir o link. Ex: (({meustatus} == 1) && ({tipo} > 200)) OU 1 para sempre exibir.");
    ...
    $form->adicionarSubTitulo("Outras Op��es");
    $arrCheckNowRap[] = array('t', ' N�o habilitar quebra de linha (nowrap)', $lslcnowrap);
    ...
    $arrCheckVisivel[] = array('t', ' Coluna � Vis�vel ', $lslcvisivel);
    ...
    $arrCheckCalculada[] = array('t', ' Coluna � calculada, com Totalizador e Sub-Totalizador', $lslccalculada);
    $form->adicionarCheckBox("lslccalculada","",$arrCheckCalculada,false);

    $arrCheckCheck[] = array('t', ' Possui Checkbox para sele��o de itens.', $lslccheckbox);
    ...
    $sql = "SELECT 
                ...
                lslcsubtotalizador_condicao,
                lslccheckbox_condicao, 
                case when lslctipo = 'int' then 'N�mero' when lslctipo = 'hora' then 'Hora' when lslctipo = 'text' then 'Texto' when lslctipo = 'data' then 'Data' when lslctipo = 'moeda' then 'Moeda' end as novo_tipo  
            FROM 
                listagem.listagem_coluna 
            WHERE 
                lslclstoid=$lstoid 
    ...
    $listagem->adicionarIndicador("indvisivel","({lslcvisivel} == 't')","I","v","Coluna Vis�vel");
    $listagem->adicionarIndicador("indcsv","({lslcexibe_csv} == 't')","I","v","Exibir no CSV");
    $listagem->adicionarIndicador("indnowrap","({lslcnowrap} == 't')","I","v","N�o habilitar quebra de Linha");
    ...
    $listagem->adicionarColuna("lslcvisivel","Vis�vel","{indvisivel}","text","center","10px");
