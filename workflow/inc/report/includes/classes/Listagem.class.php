...
class Listagem {
    ...
    /**
     * Exibe Header das Tables. (T�tulos das Colunas).
     */
    ...
    /**
     * Alterado para Protegido n�o ser� instanciado na classe.
     */
    ...
    /**
     * Indica se este objeto � uma SubListagem.
     */
    ...
    /**
     * Repetir T�tulo do Agrupamento.
     */
    private $_repetirtituloagrupamento = true;

    /**
     * Quantidade de Registros da Listagem Principal (Usada somente quando a listagem � uma sublistagem.)
     */
    ...
    /**
     * Exibir Resultados na tela. (Usado quando somente ser� exibido os resultados em CSV).
     */
     ...
     * @titulo T�tulo da Listagem.
     * @conexao Conex�o com o Banco de Dados.
     * @subListagem Indica se a Listagem � uma SubListagem. (default: false)
     */
    ...
    /**
     * Indica se a Listagem � uma SubListagem.
     *
    ...
    public function carregarIDListagem($idlistagem,$previsualizacao = false) {
        ...
        if (pg_num_rows($resu)) {
            ...
            if (($lstexclusao) && ($arquivo != "null")) {
            	$this->_debug = true;
                $this->adicionarErro("Esta Consulta est� inativada, contate o suporte t�cnico de sistemas.",true);
            }
        ...
        /*    if ($arquivo != "null") {
           ...
           // if ($previsualizacao) { $addsql = " limit 20"; }
        	...
        	$this->_debug = true;
            $this->adicionarErro("Listagem n�o encontrada.");
        }
    ...
    /**
     * Fun��o para Alterar a Mensagem de Registros Encontrados.
     *
     * @param $msg
     */
    ...
    /**
     * Fun��o para Alterar a Mensagem de Total de Registros Encontrados.
     *
     * @param $msg
     */
    ...
    /**
     * Fun��o para Alterar a Mensagem de Sub-Total de Registros Encontrados
     *
     * @param $msg
     */
    ...
    /**
     * Fun��o para Alterar a Mensagem de "Nenhum Resultado Encontrado".
     *
     * @param $msg
     */
	...
	/**
	 * Fun��o que Indica se a Listagem � Agrupada Alfabeticamente
	 *
    ...
    /**
     * Fun��o para Alterar a Op��o de Exibir os Sub-Totais de uma Listagem.
     *
     * @param $value (true ou false)
     */
    ...
    /**
     * Fun��o para Alterar a Op��o de Exibir a Quantidade de Registros Retornados da Listagem.
     *
    ...
    /**
     * Fun��o para adicionar uma Coluna na Listagem.
     *
     * @param $idcoluna
     * @param $tipo
     * @param $titulo
    ...
    /**
     * Fun��o para Adicionar uma Coluna com CheckBoxes.
     *
     * @param $idcoluna
     * @param $titulo
     * @param $html
    ...
    /**
     * Fun��o para Desenhar a Listagem.
     *
     * @return HTML
     */
    function desenhar() {
...
<link rel=\"stylesheet\" href=\"$addpath/includes/css/calendar.css\" media=\"screen\"></link>\n\n";
    ...
    /**
     * Fun��o que desenha o cabe�alho da Listagem.
     *
     * @return HTML
     */
...
'
    ...
    /**
     * Fun��o que desenha o HTML que fecha a listagem.
     *
     * @return HTML
     */
    ...
    public function adicionarParametro($idparametro,$tipo,$obrigatorio = false,$valor = "",$valortestes = "") {
        ...
        if ($this->_debug) {
        	if (($tipo != "text") && ($tipo != "int") && ($tipo != "data")) {
        		$this->adicionarErro("Par�metro '$idparametro' do tipo '$tipo' n�o � v�lido.");
                return false;
        	}
        }
        ...
        if (!$this->isParametro($idparametro)) {
            ...
            if ($this->_debug) {
        	   $this->adicionarErro("Par�metro '$idparametro' j� est� adicionado a Listagem.");
            }
        }
    }
    ...
    protected function formataSQLParametros() {
        ...
        foreach ($parametrosconsulta as $parametro_consulta) {
            ...
            if (!$achou) {
            	$this->_debug=true;
                $this->adicionarErro("Par�metro '$parametro_consulta' usado na consulta n�o foi adicionado no cadastro.",true);
            }
            if (!$achou_consulta) {
                $this->_debug=true;
                $this->adicionarErro("Par�metro '$par_base' cadastrado n�o � usado na consulta.",true);
            }
        }
    	...
    	foreach ($this->arrparametros as $parametro) {
            ...
            if (($obrigatorio == "t") && ($valor == "")) {
                $this->_debug=true;
            	$this->adicionarErro("Par�metro '$idparametro ' � obrigat�rio.",true);
            }
    ...
    /**
     * Fun��o para desenhar as Linhas da Listagem.
     *
     * @return HTML
     */
    protected function escreveLinhas() {
        ...
        if (count($this->dados)) {
            ...
            for ($qtd_reg = 1;$qtd_reg <= $qtd_registros; ++$qtd_reg) {
                ...
                //LINHA ADICIONAL DE OBSERVA��ES, (DETALHAMENTO)
   ...
   /*
       EXIBIR OS BOT�ES DE + E - PARA QUANDO POSSUIR COLUNAS COM CHECKBOXES. COMENTADO POR QUE N�O FUNCIONA SE TIVER MAIS DE UM FORMUL�RIO NA MESMA JANELA. */
        if (!$this->issublist) {
            if ($temcheck) {
                if (count($this->colunas)) {
        ...
        if ($this->_temarquivocsv) {
        	if (!$this->issublist) {
		        ...
		        if ($cnt != 0) {
			        echo   "\n\t\t<tr class='tableRodapeModelo3'>\n\t\t\t<td colspan='$colspan' align=center>";
			        //echo   '<input type="button" name="btn_arquivoxls" value="Vers�o em XLS" class="botao" onclick="window.open(\'gera_csv_to_excel.php?arquivocsv=/' . $link . '\');" style="width: 90px;">';
    ...
    protected function escreveErros() {
        if ($this->_debug) {
            ...
            echo "MODO DEBUG EST� DESATIVADO.";
        }
    }
    ...
    /*
     * Fun��o utilizada pra somar intervalos de Horas */
    ...
    /**
     * Fun��o que Retorna se a listagem Possui um checkBox adicionado.
     */
    ...
    /**
     * Fun��o que retorna o array com os Totalizadores.
     */
    ...
    /**
     * Fun��o que Retorna o array com os Sub-Totalizadores.
     */
    ...
    /**
     * Fun��o Interna Utilizada quando a Listagem possui uma sublistagem para guardar quantos registros a listagem principal tem.
     */
    ...
    /**
     * Fun��o Interna Utilizada quando a Listagem possui uma sublistagem para retornar quantos registros a listagem principal tem.
     */
