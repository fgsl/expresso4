  ...
  \**************************************************************************/
	...
	class sessions extends sessions_
	{
		...
		function clean_sessions()
		{
			...
			GlobalService::get('phpgw')->db->query("DELETE FROM phpgw_sessions WHERE session_dla <= '" . (time() - GlobalService::get('phpgw_info')['server']['sessions_timeout'])
			...
			GlobalService::get('phpgw')->db->query("DELETE FROM phpgw_app_sessions WHERE session_dla <= '" . (time() - GlobalService::get('phpgw_info')['server']['sessions_timeout'])
				. "'",__LINE__,__FILE__);
		}

		function new_session_id()
		{
			return md5(GlobalService::get('phpgw')->common->randomstring(15));
		}

		function register_session($login,$user_ip,$now,$session_flags)
		{
			GlobalService::get('phpgw')->db->query("DELETE FROM phpgw_sessions WHERE session_id='$this->sessionid'",__LINE__,__FILE__);

			GlobalService::get('phpgw')->db->query("INSERT INTO phpgw_sessions VALUES ('" . $this->sessionid
				. "','".$login."','" . $user_ip . "','"
		...
		function update_dla()
		{
			...
			GlobalService::get('phpgw')->db->query("UPDATE phpgw_sessions SET session_dla='" . time() . "', session_action='$action' "
				. "WHERE session_id='" . $this->sessionid."'",__LINE__,__FILE__);

			GlobalService::get('phpgw')->db->query("UPDATE phpgw_app_sessions SET session_dla='" . time() . "' "
		...
		function destroy($sessionid, $kp3)
		{
			...
			GlobalService::get('phpgw')->db->query("DELETE FROM phpgw_sessions WHERE session_id='"
				. $sessionid . "'",__LINE__,__FILE__);
			GlobalService::get('phpgw')->db->query("DELETE FROM phpgw_app_sessions WHERE sessionid='"
				. $sessionid . "'",__LINE__,__FILE__);
			$this->log_access($this->sessionid);	// log logout-time

			// Only do the following, if where working with the current user
			if ($sessionid == GlobalService::get('phpgw_info')['user']['sessionid'])
			{
				$this->clean_sessions();
			}
...
//					echo 'appsession returning: '; _debug_array($data);
			...
			{
				GlobalService::get('phpgw')->db->query("SELECT content FROM phpgw_app_sessions WHERE "
					. "sessionid = '".$this->sessionid."' AND loginid = '".$this->account_id."'"
					. " AND app = '".$appname."' AND location = '".$location."'",__LINE__,__FILE__);

				$encrypteddata = GlobalService::get('phpgw')->crypto->encrypt($data);
				$encrypteddata = GlobalService::get('phpgw')->db->db_addslashes($encrypteddata);

				if (GlobalService::get('phpgw')->db->num_rows()==0)
				{
					GlobalService::get('phpgw')->db->query("INSERT INTO phpgw_app_sessions (sessionid,loginid,app,location,content,session_dla) "
						. "VALUES ('".$this->sessionid."','".$this->account_id."','".$appname
						. "','".$location."','".$encrypteddata."','" . time() . "')",__LINE__,__FILE__);
				}
				else
				{
					GlobalService::get('phpgw')->db->query("UPDATE phpgw_app_sessions SET content='".$encrypteddata."'"
