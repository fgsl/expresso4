<?php
	...
	*/
	function lang($key,$m1='',$m2='',$m3='',$m4='',$m5='',$m6='',$m7='',$m8='',$m9='',$m10='')
	{
		...
		function get_theme()
		{
			...
			// se o cookie foi definido coloca tema na sess�o
			if (!empty($test_cookie))
			{
				$_SESSION['THEME'] = $test_cookie;
			}

			// se tema n�o estiver definido na sess�o retorna GlobalService::get('phpgw_info')['user']['preferences']['common']['theme']
			if ( !( isset( $_SESSION['THEME'] ) && $_SESSION['THEME'] ) )
			{
				return GlobalService::get('phpgw_info')['user']['preferences']['common']['theme'];
			}

			// sen�o retorna o tema definido na sess�o
	...
	* Multi-Domain support                                                       *
	...
	if (isset($_POST['login']))	// on login
	{
		...
		{
			GlobalService::get('login') .= '@' . get_var('logindomain',array('POST'),GlobalService::get('phpgw_info')['server']['default_domain']);
		}
	...
	{
		GlobalService::get('phpgw_info')['user']['domain'] = get_var('domain', array('GET', 'COOKIE'), FALSE);
	}
	...
	{
		unset (GlobalService::get('phpgw_domain')); // we kill this for security reasons
	}
	 ...
	 * These lines load up the API, fill up the $phpgw_info array, etc            *
	 \****************************************************************************/
	 /* Load main class */
	GlobalService::get('phpgw') = CreateObject('phpgwapi.phpgw');
	 /************************************************************************\
	...
	if (!
	GlobalService::get('phpgw')->db->connect(
		GlobalService::get('phpgw_info')['server']['db_name'],
		GlobalService::get('phpgw_info')['server']['db_host'],
		GlobalService::get('phpgw_info')['server']['db_port'],
		GlobalService::get('phpgw_info')['server']['db_user'],
		GlobalService::get('phpgw_info')['server']['db_pass'],
		GlobalService::get('phpgw_info')['server']['db_type']
	) )
	...
	* Required classes                                                       *
	...
	if (! isset(GlobalService::get('phpgw_info')['flags']['included_classes']['error']) ||
		! GlobalService::get('phpgw_info')['flags']['included_classes']['error'])
	{
	...
	* Forcing the footer to run when the rest of the script is done.             *
	\****************************************************************************/
	register_shutdown_function(array(GlobalService::get('phpgw')->common, 'phpgw_final'));
	...
	* the currentapp is not login or logout                                    *
	...
	else
	{
		...
		GlobalService::get('phpgw_info')['user']['private_dir'] = GlobalService::get('phpgw_info')['server']['files_dir']
			. '/users/'.GlobalService::get('phpgw_info')['user']['userid'];
		...
		if (!include(PHPGW_SERVER_ROOT . '/phpgwapi/themes/' . get_theme() . '.theme'))
		{
			if(!include(PHPGW_SERVER_ROOT . '/phpgwapi/themes/default.theme'))
			{
				...
				GlobalService::get('phpgw')->log->write(array('text'=>'F-Abort, No themes found'));
		...
		* If they are using frames, we need to set some variables                 *
		\*************************************************************************/
		if (((isset(GlobalService::get('phpgw_info')['user']['preferences']['common']['useframes']) &&
			GlobalService::get('phpgw_info')['user']['preferences']['common']['useframes']) &&
			GlobalService::get('phpgw_info')['server']['useframes'] == 'allowed') ||
			(GlobalService::get('phpgw_info')['server']['useframes'] == 'always'))
		{
		...
		* Verify that the users session is still active otherwise kick them out   *
		...
		if (GlobalService::get('phpgw_info')['flags']['currentapp'] != 'home' &&
			...
			GlobalService::get('phpgw_info')['flags']['currentapp'] != 'mobile')
		{
			// This will need to use ACL in the future
			if (! GlobalService::get('phpgw_info')['user']['apps'][GlobalService::get('phpgw_info')['flags']['currentapp']] ||
				(@GlobalService::get('phpgw_info')['flags']['admin_only'] &&
				! GlobalService::get('phpgw_info')['user']['apps']['admin']))
			{
				...
				GlobalService::get('phpgw')->log->write(array('text'=>'W-Permissions, Attempted to access %1','p1'=>GlobalService::get('phpgw_info')['flags']['currentapp']));
