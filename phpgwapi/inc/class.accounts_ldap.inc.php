<?php
	...
	* Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA            *
	...
	class accounts_
	{
		...
		function accounts_()
		{
			$this->ds = GlobalService::get('phpgw')->common->ldapConnect();
			if(!$this->ds)
				$this->ds = GlobalService::get('phpgw')->common->ldapConnect();
			if(!@is_object(GlobalService::get('phpgw')->translation))
			{
				GlobalService::get('phpgw')->translation = CreateObject('phpgwapi.translation');
			}
			$this->user_context  = GlobalService::get('phpgw_info')['server']['ldap_context'];
			$this->group_context = GlobalService::get('phpgw_info')['server']['ldap_group_context'];
		}
		...
		function read_repository()
		{
			...
			if($acct_type =='g')
			{
				...
				$this->data['firstname']    = GlobalService::get('phpgw')->translation->convert($allValues[0]['cn'][0],'utf-8');
			...
			else
			{
				...
				$this->data['firstname']   = GlobalService::get('phpgw')->translation->convert($allValues[0]['givenname'][0],'utf-8');
				$this->data['lastname']    = GlobalService::get('phpgw')->translation->convert($allValues[0]['sn'][0],'utf-8');
			...
			$this->data['fullname']    = GlobalService::get('phpgw')->translation->convert($allValues[0]['cn'][0],'utf-8');
		...
		function save_repository()
		{
			...
			if($acct_type == 'u')
			{
				...
				$newData['cn'] = GlobalService::get('phpgw')->translation->convert(sprintf("%s %s",
					$this->data['firstname'],
					$this->data['lastname']),GlobalService::get('phpgw')->translation->charset(),'utf-8'
				);
				$newData['uid'] = GlobalService::get('phpgw')->translation->convert(
					$this->data['account_lid'],
					GlobalService::get('phpgw')->translation->charset(),'utf-8'
				);
				if($this->data['lastname'])
				{
					$newData['sn'] = GlobalService::get('phpgw')->translation->convert(
						$this->data['lastname'],
						GlobalService::get('phpgw')->translation->charset(),'utf-8'
					);
				}
				...
				{
					$newData['givenname'] = GlobalService::get('phpgw')->translation->convert(
						$this->data['firstname'],
						GlobalService::get('phpgw')->translation->charset(),'utf-8'
					);
				}
			...
			{
				// data for posixgroup
				$newData['cn'] = GlobalService::get('phpgw')->translation->convert(
					$this->data['account_lid'],
					GlobalService::get('phpgw')->translation->charset(), 'utf-8'
				);
			...
			*/
			...
			if(GlobalService::get('phpgw')->translation->convert($test,'utf-8') != $this->data['account_lid'])
			{
				$oldData = $allValues[0];
				$oldDN   = $oldData['dn'];
		...
		function get_list($_type='both', $start = '',$sort = '', $order = '', $query = '', $offset = '',$query_type='')
		{
			...
			if($_type == 'accounts' || $_type == 'both')
			{
				...
				while (list($null,$allVals) = @each($allValues))
				{
					...
					$test = @$allVals['uid'][0];
					if (!GlobalService::get('phpgw_info')['server']['global_denied_users'][$test] && $allVals['uid'][0])
					{
						$accounts[] = Array(
							'account_id'        => $allVals['uidnumber'][0],
							'account_lid'       => $allVals['uid'][0],
							'account_type'      => $allVals['phpgwaccounttype'][0],
							'account_firstname' => GlobalService::get('phpgw')->translation->convert($allVals['givenname'][0],'utf-8'),
							'account_lastname'  => GlobalService::get('phpgw')->translation->convert($allVals['sn'][0],'utf-8'),
			...
			if ($_type == 'groups' || $_type == 'both')
			{
				...
				while (list($null,$allVals) = @each($allValues))
				{
					settype($allVals,'array');
					$test = $allVals['cn'][0];
					if (!GlobalService::get('phpgw_info')['server']['global_denied_groups'][$test] && $allVals['cn'][0])
					{
						$accounts[] = Array(
			...
			if (is_array($sortedAccounts))
			{
				...
				elseif(is_int($start))
				{
					return array_slice($sortedAccounts, $start, GlobalService::get('phpgw_info')['user']['preferences']['common']['maxmatchs']);
				}
				else
				{
		...
		function create($account_info,$default_prefs=True)
		{
			...
			if (GlobalService::get('phpgw_info')['server']['ldap_extra_attributes'] && $account_info['account_type'] != 'g')
			{
				$entry['homedirectory'] = $account_info['homedirectory'] && $account_info['homedirectory'] != GlobalService::get('phpgw_info')['server']['ldap_account_home'] ? $account_info['homedirectory'] : GlobalService::get('phpgw_info')['server']['ldap_account_home'].SEP.$account_info['account_lid'];
				$entry['loginshell'] = $account_info['loginshell'] ? $account_info['loginshell'] : GlobalService::get('phpgw_info')['server']['ldap_account_shell'];
			}
			...
			if ($allValues[0]['dn'])
			{
				...
				else
				{
					...
					$tmpentry['userpassword']   = GlobalService::get('phpgw')->common->encrypt_password($account_info['account_passwd'],False);
			...
			else
			{
				...
				if ($account_info['account_type'] == 'g')
				{
					...
					$entry['cn']             = GlobalService::get('phpgw')->translation->convert($account_info['account_lid'],GlobalService::get('phpgw')->translation->charset(),'utf-8');
					$entry['gidnumber']      = $account_id;
					#$entry['userpassword']   = GlobalService::get('phpgw')->common->encrypt_password($account_info['account_passwd']);
				...
				else
				{
					...
					$entry['cn'] = GlobalService::get('phpgw')->translation->convert(
						sprintf(
							"%s %s",
					...
					$entry['sn'] = GlobalService::get('phpgw')->translation->convert(
						$account_info['account_lastname'],
						GlobalService::get('phpgw')->translation->charset(),
						'utf-8'
					);
					...
					{
						$entry['givenname'] = GlobalService::get('phpgw')->translation->convert(
							$account_info['account_firstname'],
							GlobalService::get('phpgw')->translation->charset(),
							'utf-8'
						);
					...
					{
						$entry['mail'] = GlobalService::get('phpgw')->translation->convert(
			...
			if($account_id && is_object(GlobalService::get('phpgw')->preferences) && $default_prefs)
			{
				GlobalService::get('phpgw')->preferences->create_defaults($account_id);
			}
		...
		function get_account_name($account_id,&$lid,&$fname,&$lname)
		{
			...
			if($acct_type =='g')
			{
				$lid   = GlobalService::get('phpgw')->translation->convert($allValues[0]['cn'][0],'utf-8');
				$fname = GlobalService::get('phpgw')->translation->convert($allValues[0]['cn'][0],'utf-8');
				$lname = lang('Group');
			}
			else
			{
				$lid   = GlobalService::get('phpgw')->translation->convert($allValues[0]['uid'][0],'utf-8');
				$fname = GlobalService::get('phpgw')->translation->convert($allValues[0]['givenname'][0],'utf-8');
				$lname = GlobalService::get('phpgw')->translation->convert($allValues[0]['sn'][0],'utf-8');
			}
			return !empty($lid);
		}
